use crate::{fp::Fp, fp12::Fp12, fp2::Fp2, fp6::Fp6};
use num_bigint::{BigInt, BigUint};
use num_integer::{ExtendedGcd, Integer};
use std::str::FromStr;

const BLS_S: u8 = 2;
const BLS_K: u8 = 12;

// p = 5044125407647214251
const BLS_P: u64 = 0x4600_5555_5555_aaab;

// p^12
const BLS_P_12: &str = "271287951493331054216203954317970671853614298719053246459320624084466513613002446738951229227112346821205122843866916274880448596929078281803518339327194208026341217588466302380113683133927692384869984552794996875450399946001";

// r
const BLS_R: &str = "52435875175126190479447740508185965837690552500527637822603658699938581184513";

// h3
const BLS_H3: &str = "";

// $\lambda$
const BLS_LAMBDA: &str = "4002409555221667393417789825735904156556882819939007885332058136124031650490837864442687629129030796414117214202539";

const COEFFS_27TH_ROOT: [Fp; 12] = [
    Fp([
        0x10f6_f41b_468b_cf87,
        0x9fb9_6ccd_599f_fc30,
        0xb4ae_8194_dd7e_fb65,
        0xfd01_e4bd_4083_2fed,
        0xcd41_56cf_3a0f_efab,
        0x982c_6740_e7ed_f2cd,
    ]),
    Fp([
        0x0d7a_239f_903e_c63f,
        0x697a_66ef_bda2_1c56,
        0x0401_68d6_b1fb_4342,
        0xb66f_892f_a130_224c,
        0xa3a5_81f7_e564_876e,
        0x81fc_7694_73bd_85f7,
    ]),
    Fp([
        0x0253_e2e2_c93b_5511,
        0xdf24_7ce0_0a70_5ed5,
        0x506e_c40d_cf3e_2971,
        0xd0f0_c5b4_36a3_56db,
        0xd22e_3deb_a7b2_184d,
        0x6661_6181_0998_96f7,
    ]),
    Fp([
        0x03d9_75b3_6aaf_2e81,
        0x0ace_ec99_2bcf_b2b9,
        0x217b_f843_b186_3214,
        0x6d54_0b52_10b0_6307,
        0x824c_fcbf_76d4_2c4d,
        0x6a65_0edb_e934_5d0a,
    ]),
    Fp([
        0x0329_9b39_cd50_cd55,
        0x4a44_7121_028d_b683,
        0xb341_ae1a_9044_b1bb,
        0x0c2e_2b23_dfbe_dd1c,
        0x6656_1a1e_dc9e_f823,
        0xc129_4563_2d5f_1677,
    ]),
    Fp([
        0x0307_f9cd_4ed9_168d,
        0xa147_a20a_be8b_a293,
        0xaf90_ef6a_914a_dc99,
        0x2fdd_70b4_24e1_b2a2,
        0xa57d_b912_3be5_2fad,
        0x74a0_8331_2987_5f72,
    ]),
    Fp([
        0x002a_ece6_4f92_e438,
        0x928a_f203_e32f_7556,
        0xb1f2_e9f7_bb2f_4aba,
        0xf36d_b24f_72e6_5519,
        0x3663_1c89_fa6c_6a92,
        0x301e_a12f_bd37_1ffc,
    ]),
    Fp([
        0x146e_b2f7_201f_14c8,
        0xf836_b147_4d99_486c,
        0x7857_2c83_b595_70a8,
        0xeb3e_9f79_2ebf_8147,
        0x61cb_b8fc_519e_00b2,
        0x4467_612e_d9a0_b71d,
    ]),
    Fp([
        0x0fd5_a632_6241_511e,
        0xb3d1_1db0_4e54_0a77,
        0x8470_e309_9eb9_5b15,
        0x8e5e_24ae_4b4a_f029,
        0xf890_b0b0_025b_30fa,
        0x6def_2091_de4c_3daa,
    ]),
    Fp([
        0x02d5_7a27_d040_5489,
        0x3441_e6c0_ddd6_fd65,
        0x70b2_50e6_40f6_2e25,
        0xe06f_d45c_a115_2673,
        0x3e8e_32d8_430c_1fa2,
        0x350a_5913_850f_7b26,
    ]),
    Fp([
        0x0fed_fce9_0768_13a3,
        0x0917_ecca_858d_0086,
        0x9112_ac38_cfe5_1858,
        0xc601_4c9c_ae70_acfb,
        0x5941_6249_d441_3771,
        0x36c0_b31d_625a_5a89,
    ]),
    Fp([
        0x1033_41bc_eb3a_c0d1,
        0xbce4_248f_fb92_b258,
        0xa20f_ba1e_2cc2_629c,
        0x5f19_1735_dbdb_d30b,
        0xa9b4_abb2_6fb0_a8b1,
        0xb45d_3cb2_b9df_54ed,
    ]),
];

/// 27th root of Fp12
/// 2215832431738254021858430833473491971472907921681848639066176995827469549541336756374498109335286165907308496272036*u^11 + 1568830231290002655414696898990994515906754827216931272303146338935885249207435695220858508099237158006327999983721*u^10 + 2789190503201674451857891076016074209873756838728155128589046353191958032604279347727174717406170552051958929605662*u^9 + 2766757639517403038926022128169213511408030714287506907840740953445619999258272434055733105667304591457836852294939*u^8 + 1899290885966997915932727775886289454766769693831016185819397553336201976561994577812375445396848740716768661066906*u^7 + 2730521554304824852333211249086101430803514712548201970920079722904463249643031119531359650834655731163153266539482*u^6 + 2372335291936719397558586286573991991990943761742261105882728734115101996471396939598874216892270954654873982192915*u^5 + 2647761599626775211506645008910551257291866501828740380582086965940927532121673610525201471087360012327636080951779*u^4 + 389437429987139622466189702971416088270898314622679253440470651889296222661007011295568998742161246291242055447985*u^3 + 952087828372006056375175265475346405803455327684867402998952995038578055919629631662111846585901856698458366123906*u^2 + 110700656196224825338140726992030708352686271264597717261216695317674463693221521918984355795975394087767305652271*u + 1587876598240339177559172903511792795433185889988342843144628184734599371629235594140230491168556177222480807631015
const W27: Fp12 = Fp12 {
    c0: Fp6 {
        c0: Fp2 {
            c0: COEFFS_27TH_ROOT[0],
            c1: COEFFS_27TH_ROOT[2],
        },
        c1: Fp2 {
            c0: COEFFS_27TH_ROOT[4],
            c1: COEFFS_27TH_ROOT[6],
        },
        c2: Fp2 {
            c0: COEFFS_27TH_ROOT[8],
            c1: COEFFS_27TH_ROOT[10],
        },
    },
    c1: Fp6 {
        c0: Fp2 {
            c0: COEFFS_27TH_ROOT[1],
            c1: COEFFS_27TH_ROOT[3],
        },
        c1: Fp2 {
            c0: COEFFS_27TH_ROOT[5],
            c1: COEFFS_27TH_ROOT[7],
        },
        c2: Fp2 {
            c0: COEFFS_27TH_ROOT[9],
            c1: COEFFS_27TH_ROOT[11],
        },
    },
};

pub const WP: Fp12 = Fp12 {
    c0: Fp6 {
        c0: Fp2 {
            c0: Fp([
                0x048fd3da7e6bff25,
                0x579b2d13725fab21,
                0x22a0636bba43ca4c,
                0x899380ca15caff7f,
                0x097d8788dd84085f,
                0x4068bfb0ba6c60d4,
            ]),
            c1: Fp::zero(),
        },
        c1: Fp2::zero(),
        c2: Fp2::zero(),
    },
    c1: Fp6::zero(),
};

// (q^k - 1)/(gcd(3, q^k - 1)) for checking if a field element in Fp12 has a 27th root
const HAS_27TH_ROOT_EXP: [u64; 72] = [
    0x0000000000713a91,
    0x7ede2bd1095d5c4a,
    0x3ba6a646860b4bd9,
    0x136846fde9ab4bb1,
    0x2b27746308c5cc35,
    0x5c29f2b22bbbf0ac,
    0x31f5a7f0678f800d,
    0x71e631efc41fe1ac,
    0x303159aca27dc7a8,
    0xa2bede9feeb424cf,
    0x402669682179b329,
    0x1820df99020f31ad,
    0x80271bd78fe70821,
    0x1ec321f212b61e5f,
    0x1018c3b2c9f65748,
    0xc2125611f8c41898,
    0x0df9dd5d14aeaf3e,
    0x0c011995bbd2382d,
    0x631877e192ee5990,
    0xdeaacafe1e8d5d5b,
    0xbd421fa96d51d326,
    0x76ba1e2bffa76142,
    0x2fd6d0d0de38f9e6,
    0xe4d8c44d3544a2f8,
    0xb3bb7f6ae43bbaa0,
    0xbd398cf79ddf9022,
    0x23abfd5966b17711,
    0x3148c447fe65c0a0,
    0xfd669549adc74650,
    0xaa88e9ef6497eae1,
    0xe7a35740f4d57330,
    0x949efd7cbfa089da,
    0xaa1413c5ce3fbbe7,
    0x6c756380cd6c8d64,
    0x9cf41f4fdbe1f211,
    0x19dc1b482a4cd100,
    0xe152f8bdaf8aa6ea,
    0x5fdd9aea2415c2b2,
    0x6550450fffc047a8,
    0xaa41a31a9897e6a9,
    0xf6c02beb9ac81951,
    0xe28eb0b3e143fee5,
    0x3f72fc97abf0e803,
    0x994e936c6569478a,
    0x40db4ff7584076a8,
    0xba186f9a10a0910e,
    0x4c5108282bf94015,
    0x4a72fd9e0b17a18f,
    0xcd443ede7a9ca099,
    0xb6a640ee355f6388,
    0x97f8ab0e3bccb6c9,
    0x076dcc348920288a,
    0xeb3f50f1f0625e07,
    0x8ec84e5d427523fd,
    0xee8f73d5b551fd96,
    0x1ca48c80409560a2,
    0xa6bb2d19506e674b,
    0xd1cc777f59fcc581,
    0x632e45a026ce2798,
    0x868bc704aad9b7ac,
    0xfb05fbf5af355964,
    0x95e878ea1561e23e,
    0xe2b42f0535374732,
    0x5108809d45a1a1ac,
    0xddf0fae6bf2f08ca,
    0xe3ae2a1d637e74ad,
    0x55295e79ce9e40c3,
    0x8c50aca42377b1ab,
    0x796d0e6154ae16ac,
    0x4f51ed5b2826c043,
    0x9be5f62006992eca,
    0x76426c371f51d1b0,
];

// (q^k - 1)/(gcd(p, q^k - 1)) for checking if a field element in Fp12 has a pth root
const HAS_P_ROOT_EXP: [u64; 71] = [
    0x0000000004da4226,
    0x095724c2ef911566,
    0x326ac0491276a157,
    0xe5efe09927c3051a,
    0x7ad129d1a3ae5f6b,
    0x8cb36b5a56bd3237,
    0x93f3486c47843429,
    0x5fc40bdaa7ba2646,
    0xdb3b3d43dcd5845f,
    0x37e22421f9e5e41f,
    0xb374e42c65484d82,
    0x618b34e1ba582df5,
    0xedb35758c266e8f6,
    0x20bbb0cbf1aac576,
    0x37edbf429b265de3,
    0x573e937a25088c68,
    0xdea9ae71cd1bf777,
    0x598eb5d30c2dd978,
    0x2aad75551a15faf4,
    0x14c616d8df2ff7d8,
    0x663a367c16617c38,
    0x2b698ef4da9bedd1,
    0xb4dceb2c055e062d,
    0x96709bb71fe183f4,
    0x8cc6f1d7167586a7,
    0xc79ea6bfbcf29e72,
    0xd6d4f0e6c7da2525,
    0x8e3c7e38b74acb8f,
    0xed35c68ac15a3411,
    0x2d1902f47ff57d57,
    0x1ee6ee48c2f36aaf,
    0x3f0c285ab2c74bcb,
    0xebe514a259075d27,
    0x8df337f90a8ea46b,
    0x66b14c4b464e40d5,
    0x47d3063ba769087d,
    0x0ddca9a592afcbd3,
    0xf0b4bf9fe1115896,
    0x95d32848aa727edd,
    0x8f08f6741a4cbde5,
    0xaec674baafacd5fb,
    0x1caf7dd8e02ad59b,
    0xc905c36850ce49fb,
    0xcc0197e1f8b8a250,
    0xc4e21b12c33cf5a1,
    0xc708df2bdfd24580,
    0xede244eb6f909959,
    0xfd6cc3d60d8143fd,
    0xccfaa04e771af56d,
    0xc1fcdf0e1e305a73,
    0x079e783e5e40368c,
    0xdbf945f28ca7fd33,
    0x61a6929793841841,
    0xcb05de0bf1bc42f0,
    0xb81deace53624411,
    0x31bb206982e95b70,
    0x604d7f1a29f85c97,
    0x5591517a4b651fb0,
    0x68b4f036ddce02a2,
    0x96610aafa66f3050,
    0x7742aea2e96fa2c6,
    0x141df64be039bd11,
    0x4c9f1ade0d25136c,
    0xf1d710ac328439a7,
    0xb3921c77b1ff9e5d,
    0x58a630a970aa4c16,
    0x236bf33da7b4a560,
    0x8bf14557600a9451,
    0x8528a46cf56fd582,
    0x2f489ae3fa8be672,
    0x55e6133fbab05f30,
];

const ZETA_FP12_LAMBDA: Fp12 = Fp12 {
    c1: Fp6 {
        c2: Fp2 {
            c1: Fp([
                0x065a92f719ab3e7d,
                0x220c03b436e349c5,
                0xc5aae1e726ed0c0e,
                0x5a25458ccd10b12d,
                0xaeb5556bff090a4e,
                0xe26d499d3c4926d9,
            ]),
            c0: Fp([
                0x02f6dd91bb6286ee,
                0xa5743d4788397964,
                0x31f71c5a8aa28fe1,
                0x26702742db3eb498,
                0x835e8732e2602756,
                0x693b733a267bee23,
            ]),
        },
        c1: Fp2 {
            c1: Fp([
                0x03b07621222db983,
                0x381d059b6e350ac6,
                0x9bcf7349b3548913,
                0x6f7cc060dbf7b1d3,
                0x258fe17208aea00b,
                0x91f1c0461f5263e8,
            ]),
            c0: Fp([
                0x0fbc94ac5e6ce436,
                0x26f634ef4b49eee4,
                0xac5de9bfe7812478,
                0xc3009d55cb36cde6,
                0x0a80de699cd4d642,
                0x0b82360ce43562a6,
            ]),
        },
        c0: Fp2 {
            c1: Fp([
                0x0d5bd3432aef6b89,
                0x6faf5c63edc4d4f2,
                0x53c85d15ba7bc567,
                0x64f5bb3cec4755b9,
                0x0e285129c7a5901a,
                0x17552740182de4ac,
            ]),
            c0: Fp([
                0x0f1cd9358bd9d2a6,
                0x51786b742406f90f,
                0x0775aa1fd93088a4,
                0xcf13d79aaa1902b2,
                0x396238e100c1d41d,
                0x97eaa939d79dd68d,
            ]),
        },
    },
    c0: Fp6 {
        c2: Fp2 {
            c1: Fp([
                0x0ad55d0470b49a10,
                0x38ecce526c7a8d8c,
                0xc264989594c395ce,
                0x2e2630669e7bd074,
                0x3340c313e11cf2b9,
                0x3b0c667803edb567,
            ]),
            c0: Fp([
                0x097c9df0d7246bb3,
                0x9e16b4e9c6a2d98b,
                0x371237206962f390,
                0xa519b6ed41b43082,
                0xb5f13bc0e927868d,
                0x916ceb4196e402b9,
            ]),
        },
        c1: Fp2 {
            c1: Fp([
                0x01735d7eb2775296,
                0x5f326d0ae1f95105,
                0x447de766d19ac339,
                0x6f7aeac80e807aec,
                0xdd2092dd60124469,
                0xa3b18483990594d2,
            ]),
            c0: Fp([
                0x0d0423d996d8d83a,
                0x33eab08ef6299ce2,
                0x8004d1c6c240ac01,
                0x3152961fad5ab65b,
                0xafe8cf64c4e19069,
                0x38e23a02c7a44330,
            ]),
        },
        c0: Fp2 {
            c1: Fp([
                0x0a8a15f10bf308b3,
                0x68197d3789bef740,
                0x1121032c8ddb7d6d,
                0x5a3d5d2fea5fef75,
                0xba2e07476a251d60,
                0xc7a71126825620e0,
            ]),
            c0: Fp([
                0x0d5ac7f57cecd959,
                0xac32c021d4b19dcc,
                0xc49693348a16f454,
                0x84a636bbabb4668a,
                0x20ca7d37f201443b,
                0x59ec3a2f4e99f383,
            ]),
        },
    },
};

fn has_27th_root_contribution(a: Fp12) -> bool {
    a.pow_vartime(&HAS_27TH_ROOT_EXP) == Fp12::one()
}

fn has_pth_root_contribution(a: Fp12) -> bool {
    a.pow_vartime(&HAS_P_ROOT_EXP) == Fp12::one()
}

fn get_pth_root_inverse(a: Fp12) -> Fp12 {
    if has_pth_root_contribution(a) {
        return Fp12::one();
    }

    const S: [u64; 68] = [
        0x0000000001d84b98,
        0x6e90fb4dd9fc2882,
        0x8d09ebbac89f5653,
        0x98d127acb72b76b2,
        0xa777ae6586d4167d,
        0x6d9f4a514fba11b8,
        0xebe81970a4afbf7a,
        0xa338faf057eb050c,
        0x11034aa450d840e5,
        0xb8bf7ff9a1a51056,
        0xdd05fe7908fea12f,
        0x70789673d175dfc9,
        0x98956b4b639fe9fa,
        0x73fcae59a8b216b0,
        0x50a7a7850ef59a41,
        0xc4d3aca46f5fd9bd,
        0xfce6e91abe3d11a0,
        0x68c3574e325cabb1,
        0xecc5a040ac87cba7,
        0xb91b8df25899e298,
        0x952439e468e06ece,
        0xd121f46fba1198ee,
        0x5a0e35c82c7f823a,
        0x4335ce0db07f7d27,
        0xff69c65d0cabe143,
        0x04605ed686ec566f,
        0x9b45267fc8bde1f4,
        0xad9f9cbde05d1586,
        0xf65a4d96a7184dde,
        0xaebb75f397547b1e,
        0xfb1bb1d1dcf6edc4,
        0x86e495f6cefccb89,
        0x516df40312508397,
        0x7138e8f2e8532d6f,
        0xddef8b5c8da2f479,
        0xf786872c18d4b2dd,
        0x8b1d6d01321d74cb,
        0xedbf35ee9ef8394b,
        0x3600306fd22d0897,
        0x3b7ba4b233df3bff,
        0x11d764d2904dfb4a,
        0xb7c951d691859534,
        0x96bb030ead6470a1,
        0xc9b0b311c50afc66,
        0x2de918779c1ac2a1,
        0xd00830c40730f3ee,
        0xa369554011ef241f,
        0xe9a5e683d2b5b5c4,
        0x883893d2efbbd422,
        0x21eabfabc353ac3d,
        0xe92a1e8b17221bb0,
        0x73968104a42f1b09,
        0x54c9b7c35bf815fc,
        0xab4d2784193a8548,
        0xdf3628766f63288f,
        0x654c622717af97b2,
        0x6ef05052a220a881,
        0x1fd5ce61efb63176,
        0xef7391554c05b280,
        0xf42e73808367d509,
        0x3573da7058ebce3b,
        0xc632e5f3df40a735,
        0xc057141d2569299c,
        0x9933b92ca7c176c7,
        0x33992d8b37d2c3fc,
        0x9c9c19ca65aca372,
        0xcc0ee2b67e3235d4,
        0xc53474ed740c5330,
    ];

    a.pow_vartime(&S)
}

fn get_27th_root_inverse(a: Fp12) -> Fp12 {
    if has_27th_root_contribution(a) {
        return Fp12::one();
    }

    const S: [u64; 68] = [
        0x00000000016238b2,
        0x52ecbc7a63c951cc,
        0xba369d2f2c765469,
        0x36a1d31c9145ef07,
        0x048356d6b9a3b5b3,
        0xf58f8fb0f067830d,
        0x1bae9336e4d7ccc8,
        0xc59f06eaf2c31da9,
        0xdb79bd6ae23e20bf,
        0x24282c9af326d8ce,
        0x3773091e6b7a3c52,
        0x576426b122136119,
        0x9c50fcff0cacad6f,
        0x3c7a6f8a36191277,
        0x00d172d3e2e15393,
        0xbf974232c47cb032,
        0xdbc255bcd828e275,
        0x58fa67720cea772e,
        0x2e58e80df2110645,
        0x78cb0d922fdf8765,
        0x55b9f5d176f0c674,
        0xf5b2be313915fe0d,
        0x74b0b5cb35cb55f9,
        0x79fbf6bde00221e6,
        0xa73eacab8824115b,
        0x8e33e0d7861329d2,
        0xa38a2b3c5d9ea658,
        0xd114a80e38865c27,
        0xb937df100cc73a19,
        0xb05cb556b6e87950,
        0x6698bac0e4ac6d41,
        0xbb94fbd392dd25d0,
        0x6c586eeb0976a730,
        0x67375ffbbf395d56,
        0x78108e29c5101b30,
        0x7f282577a60b5407,
        0x42b55cc58ab13619,
        0xa4a31f46e7b0ce3c,
        0xbf64ead4ef401684,
        0x91379900a9442cdc,
        0xb1f72d32bde70333,
        0xbd510ae3945f4f60,
        0xb74cbf27e444e582,
        0x36c157a078a79df1,
        0x1fdb3cdceff1396b,
        0x953bafc4867223dd,
        0xe1dd4092786edec6,
        0x9576b178ef9b6fe9,
        0x749298d15e320344,
        0xb338d847a3cdad61,
        0x47d6bce3654c84e4,
        0x6c3b86ad3a305b74,
        0x8a3a0c1686c5281e,
        0x82c0d5dfe9183972,
        0xf05ddd917c2f704d,
        0xb2597143b76b0906,
        0x1fa9389424ebd7da,
        0x3979ece07ff32fa4,
        0xaf79a84323bdb01f,
        0xefb778edc6c28895,
        0x873018ca7dedcce6,
        0x2a5f4aa3a20b0cdf,
        0x4c26f2dc8ef7e09d,
        0x3a91bd5846f43a80,
        0xe0325eed8350f964,
        0x2f60bbddaea8a280,
        0xbdfdd46d4a420827,
        0x30cf13c36a71b2d0,
    ];

    a.pow_vartime(&S)
}

/// Compute $\lambda$-th root of an element in Fp12
pub(crate) fn lambda_root(a: Fp12) -> Result<Fp12, String> {
    let mut a = a.clone();
    let q = BigUint::from(BLS_P);
    let q_minus_one = &q - BigUint::from(1_u8);
    let n = BigUint::from_str(BLS_LAMBDA).unwrap();
    let gcdd = (&q).gcd(&n);

    let one: BigUint = BigUint::from(1_u8);

    if a == Fp12::one() {
        if gcdd == one {
            return Ok(a);
        } else {
            return Ok(ZETA_FP12_LAMBDA);
        }
    }
    // gcdd != q-1 so there is always a \lambda-th root
    let ExtendedGcd { gcd, x, y } = n.extended_gcd(&q_minus_one);

    if y == one {
        return Ok(a.pow_vartime(gcd.to_u64_digits().as_slice()));
    }
    let n = y;
    let q_minus_one_over_n = q_minus_one.div_floor(&n);

    if a.pow_vartime(q_minus_one_over_n.to_u64_digits().as_slice()) == Fp12::one() {
        return Err("No lambda-th root exists".to_owned());
    }

    a = a.pow_vartime(gcd.to_u64_digits().as_slice());
    let primes: [BigUint; 6] = [
        BigUint::from(3_u128),
        BigUint::from(11_u128),
        BigUint::from(10177_u128),
        BigUint::from(859267_u128),
        BigUint::from(52437899_u128),
        BigUint::from_str(
            "52435875175126190479447740508185965837690552500527637822603658699938581184513",
        )
        .unwrap(),
    ];
    let multiplicity: [u32; 6] = [1, 2, 2, 2, 2, 1];

    primes.iter().zip(multiplicity).for_each(|(r, v)| {
        let (k, h) = padic_val_unit(&q_minus_one, r);
        let h_inv = BigInt::from(-h).modpow(
            &BigInt::from(-1),
            &BigInt::from_biguint(num_bigint::Sign::Plus, r.pow(v)),
        );

        let z = h * h_inv;
        let x = (BigInt::from(1) + z)
            .div_floor(&BigInt::from_biguint(num_bigint::Sign::Plus, r.clone()));
        if k == BigUint::from(v) {
            let sign = x.sign();
            let x = if sign == num_bigint::Sign::Minus {
                (x * BigInt::from(-1)).to_biguint().unwrap()
            } else {
                x.to_biguint().unwrap()
            };
            a = a.pow_vartime(&x.to_u64_digits().as_slice());
            if sign == num_bigint::Sign::Minus {
                a = a.invert().unwrap();
            }
        } else {
            // let gh =
        }
    });
    // let F =
    todo!()
}
